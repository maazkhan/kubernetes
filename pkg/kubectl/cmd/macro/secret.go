...
func NewCmdCreateSecret(f *cmdutil.Factory, cmdOut io.Writer) *cobra.Command {
	...
	cmd.AddCommand(NewCmdCreateSecretOpaque(f, cmdOut))
	return cmd
}
...
`
  ...
  $ kubectl create secret opaque my-secret --from-file=path/to/bar

  // Create a new secret named my-secret with specified keys instead of names on disk
  $ kubectl create secret opaque my-secret --from-file=ssh-privatekey=~/.ssh/id_rsa --from-file=ssh-publickey=~/.ssh/id_rsa.pub

  // Create a new secret named my-secret with key1=supersecret and key2=topsecret
  $ kubectl create secret opaque my-secret --from-literal=key1=supersecret --from-literal=key2=topsecret`
)

// NewCmdCreateSecretOpaque is a command to create generic secrets from files, directories, or literal values
func NewCmdCreateSecretOpaque(f *cmdutil.Factory, cmdOut io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "opaque NAME [--type=string] [--from-file=[key=]source] [--from-literal=key1=value1] [--dry-run=bool]",
		Short:   "Create a secret from a local file, directory or literal value.",
		Long:    secretLong,
		Example: secretExample,
		Run: func(cmd *cobra.Command, args []string) {
			err := CreateSecretOpaque(f, cmdOut, cmd, args)
...
// CreateSecretOpaque is the implementation the create secret opaque command
func CreateSecretOpaque(f *cmdutil.Factory, cmdOut io.Writer, cmd *cobra.Command, args []string) error {
	stringSliceParams := []string{"from-file", "from-literal"}
	return createCommandInternal(f, cmdOut, cmd, args, "secret/v1", commandParamStringSliceVisitor(stringSliceParams))
}
...
func NewCmdCreateSecretDockerRegistry(f *cmdutil.Factory, cmdOut io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "docker-registry NAME --docker-username=user --docker-password=password --docker-email=email [--docker-server=string] [--from-literal=key1=value1] [--dry-run=bool]",
	...
	cmdutil.AddPrinterFlags(cmd)
	cmd.Flags().String("generator", "", "The name of the API generator to use.  If not specified, default to secret-for-docker-registry/v1")
...
func CreateSecretDockerRegistry(f *cmdutil.Factory, cmdOut io.Writer, cmd *cobra.Command, args []string) error {
	return createCommandInternal(f, cmdOut, cmd, args, "secret-for-docker-registry/v1", nil)
}
