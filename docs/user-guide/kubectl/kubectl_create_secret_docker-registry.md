...
```
kubectl create secret docker-registry NAME --docker-username=user --docker-password=password --docker-email=email [--docker-server=string] [--from-literal=key1=value1] [--dry-run=bool]
```
...
```
      ...
      --generator="": The name of the API generator to use.  If not specified, default to secret-for-docker-registry/v1
